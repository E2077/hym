 ...
 * @LastEditors: Hellager
 ...
 点击 环境变量, 点击 新建变量，名称COOKIE_WEIBO, 自动拆分选 否, 值填抓包获取到的值，多个 COOKIE 请用 ';' 隔开，通知相关变量参考 SendNotify.js 中参数
 
 变量名称	变量含义
ROW_URL(必需)	微博国际版手机端抓包得到的原始链接
需包含aid, gsid, from, s
SIGN_TYPE(必需)	签到方式选择
DEFAULT -> 默认全部签到
ONLY -> 仅签到在SIGN_LIST中的值
EXCEPT -> 仅签到不在SIGN_LIST的值
SIGN_LIST(可选)	签到列表设置，直接输入超话名称，以 ; 间隔

DISP_TYPE(必需)	结果是否展示等级信息
DEFAULT -> 默认成功简略显示
DETAIL-> 成功签到显示等级

10 12,19 * * * Sign_Weibo.js, tag=微博超话签到脚本, enabled=true
