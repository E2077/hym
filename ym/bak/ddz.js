...
class UserInfo {
    ...
    async getuseinfo() {
        try {
            ...
            if (name)var newname=name.substr(0,1)+'*'+name.substr(2)
            alipay=result.result.alipay||'';
		  alipay=''+alipay
		  if (alipay)var newalipay = alipay.substr(0,3) + "****" + alipay.substr(7)
                console.log(`\n账号[${this.name}]用户 `+result.result.nickname+`  余额 ${this.money}元 邀请数  `+result.result.pinnum)
                console.log(`账号[${this.name}]`+`绑定支付宝账号 `+newname+` - `+newalipay)
                console.log(`账号[${this.name}]`+`今日收益`+result.result.todaymoney+`元  已提现`+result.result.cashallmoney+`元`)
if (this.money>=0.6){
